...
const currentUsersOrders = function (db, orderId, userId) {
  const queryText = {
    text: `SELECT * FROM orders WHERE orders.id=$1 AND user_id=$2`,
    values: [orderId, userId],
  };

  return db.query(queryText).then((orders) => {
    return orders.rows.length > 0 ? orders.rows[0] : null;
  });
};
